...
- `screener/divergence.py` — `detect_double_divergence` : repère une paire en
  ...
  Vraie divergence = sens OPPOSÉ prix/RSI (creux égal/plus bas + RSI plus haut ; sommet
  égal/plus haut + RSI plus bas), bande d'égalité `equal_tol_pct`.
  `detect_forming_entry` (mode **entrée live AU 2ᵉ creux**) : la barre COURANTE (non
  confirmée par fractale) teste la zone du 1er creux avec divergence RSI ; le rejet
  intra-barre (clv/volume) tient lieu de confirmation. Causal (n'utilise que `df`) →
...
- `screener/cli.py` — orchestration + sortie tableau/CSV ; `--mtf` → run_mtf,
  ...
  `--entry` → run_entry (entrée live au 2ᵉ creux : niveau limite + barre de confirmation,
  stop/T1/T2/R:R ; `--forming-only` = entrées confirmées seulement),
  `--chart` génère le PNG.
