...
- `screener/divergence.py` — `detect_double_divergence` : repère une paire en
  ...
  égal/plus haut + RSI plus bas), bande d'égalité `equal_tol_pct`. Détection pure : pas
  de stop/objectif/R:R ni de calcul de rendement (l'utilisateur choisit lui-même la cible).
