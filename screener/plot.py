...
def plot_window_structure(
    ...
    # Panneau Open Interest — bougies standard (vert = close ≥ open, rouge sinon), convention
    # TradingView. L'OHLC binance est reconstruit depuis la 1m côté data (open/high fiables),
    # donc plus de bougie « rouge qui flotte au-dessus » due à un open inventé.
    if has_oi:
        ...
        compo = f"{label} — vert=OI↑ · rouge=OI↓"
        if oi_source == "agg3":
            try:
