...
def plot_forming_entry(
    symbol: str, analysis_tf: str, res, out_path: str,
    ex=None, tz_hours: int = 2, tz_label: str = "CEST", limit: int = 1000,
    rsi_period: int = 14,
) -> str:
    """Rendu d'une entrée live AU 2ᵉ creux/sommet (`res` = FormingEntry). Panneau prix :
    bougies TF inférieure, climax (SC/BC), 1er creux (réf), barre de test courante, zone
    LIMITE, STOP, ligne de cou (T1) et objectif mesuré (T2). Panneau RSI + droite de
    divergence (réf → barre courante)."""
    from .features import add_features
    ...
    climax_ts = coarse.index[ci]
    ref_ts = res.ref.ts
    test_ts = coarse.index[-1]              # barre courante = test
    key_ts = [climax_ts, ref_ts, test_ts]

    span = max(key_ts) - min(key_ts)
    pad = span * 0.18 if span.total_seconds() else td * 5
    t0, t1 = min(key_ts) - pad, max(key_ts) + pad + td * 2
    ...
    x = mdates.date2num(sub.index.to_pydatetime())
    width = (x[1] - x[0]) * 0.7 if len(x) > 1 else 0.01
    fig, (axp, axr) = plt.subplots(2, 1, figsize=(13, 7.8), sharex=True,
                                   gridspec_kw={"height_ratios": [3, 1.3], "hspace": 0.05})
    fig.patch.set_facecolor("white")
    _candles(axp, sub, width)
    ...
    def locate(ts):
        ...
        if len(seg) == 0:
            return mdates.date2num(start.to_pydatetime()), None
        if want == "low":
            return mdates.date2num(seg["low"].idxmin().to_pydatetime()), float(seg["low"].min())
        return mdates.date2num(seg["high"].idxmax().to_pydatetime()), float(seg["high"].max())

    xc, pc = locate(climax_ts); pc = pc if pc is not None else res.ref.price
    xr_, pr = locate(ref_ts);   pr = pr if pr is not None else res.ref.price
    xt, pt = locate(test_ts);   pt = pt if pt is not None else res.test_ext

    # niveaux opérationnels
    axp.axhline(res.limit_level, color="#2e7d32", ls="--", lw=1.1, alpha=0.9)
    axp.text(x[0], res.limit_level, f" limite {res.limit_level:.4g}", va="bottom", fontsize=8, color="#1b5e20")
    axp.axhline(res.stop, color="#d62728", ls="--", lw=1.1, alpha=0.9)
    axp.text(x[0], res.stop, f" stop {res.stop:.4g}", va="top", fontsize=8, color="#a01b1b")
    axp.axhline(res.neckline, color="#b08900", ls="-", lw=1.1, alpha=0.9)
    axp.text(x[0], res.neckline, f" ligne de cou / T1 {res.neckline:.4g}", va="bottom", fontsize=8, color="#7a5c00")
    axp.axhline(res.measured, color="#b08900", ls=":", lw=1.0, alpha=0.8)
    axp.text(x[0], res.measured, f" objectif T2 {res.measured:.4g}", va="bottom", fontsize=8, color="#7a5c00")

    col_c = "#2ca02c" if acc else "#d62728"
    lbl_ref = "creux 1" if acc else "sommet 1"
    lbl_test = "TEST (2ᵉ creux)" if acc else "TEST (2ᵉ sommet)"
    same = climax_ts == ref_ts
    markers = [(xt, pt, lbl_test, "#1f77b4")]
    if same:
        markers.insert(0, (xc, pc, f"{res.climax} ({lbl_ref})", col_c))
    else:
        markers.insert(0, (xr_, pr, lbl_ref, "#1f77b4"))
        markers.insert(0, (xc, pc, res.climax, col_c))
    dy = 18 if not acc else -34
    for xx, pp, lab, c in markers:
        axp.scatter([xx], [pp], s=160, facecolor="none", edgecolor=c, lw=2.4, zorder=6)
        axp.annotate(lab, (xx, pp), textcoords="offset points", xytext=(0, dy), ha="center",
                     fontsize=9, weight="bold", color=c,
                     arrowprops=dict(arrowstyle="-", color=c, lw=0.8))

    state = "CONFIRMÉE" if res.confirmed else "limite en attente"
    axp.set_title(f"{symbol} — entrée {('LONG' if acc else 'SHORT')} au 2ᵉ "
                  f"{'creux' if acc else 'sommet'} ({state}, R:R {res.rr:.1f})  |  analyse "
                  f"{analysis_tf}, bougies {fine_tf} ({tz_label})", fontsize=11, weight="bold")
    axp.set_ylabel("Prix"); axp.grid(True, alpha=0.2)
    plo = min(float(sub["low"].min()), res.stop)
    phi = max(float(sub["high"].max()), res.measured)
    axp.set_ylim(plo - (phi - plo) * 0.06, phi + (phi - plo) * 0.08)

    # --- Panneau RSI + droite de divergence (réf → barre courante) ---
    xrr = mdates.date2num(csub.index.to_pydatetime())
    axr.plot(xrr, csub["rsi"].values, color="#6a3d9a", lw=1.1, label=f"RSI({rsi_period})")
    axr.axhline(70, color="#bbb", ls=":", lw=0.8); axr.axhline(30, color="#bbb", ls=":", lw=0.8)
    axr.axhline(50, color="#e2e2e2", lw=0.6)
    xa = mdates.date2num((ref_ts + delta).to_pydatetime())
    xb = mdates.date2num((test_ts + delta).to_pydatetime())
    div_col = "#2ca02c" if acc else "#d62728"
    axr.plot([xa, xb], [res.rsi_ref, res.rsi_now], color=div_col, ls="--", lw=2.0, zorder=5,
             label=f"divergence Δ{res.rsi_div:+.0f}")
    axr.scatter([xa, xb], [res.rsi_ref, res.rsi_now], s=70, color="#1f77b4", zorder=6)
    for xx, rv in [(xa, res.rsi_ref), (xb, res.rsi_now)]:
        axr.annotate(f"{rv:.0f}", (xx, rv), textcoords="offset points", xytext=(0, 6),
                     ha="center", fontsize=8, weight="bold", color="#1f77b4")
    axr.set_ylabel("RSI"); axr.set_ylim(0, 100); axr.grid(True, alpha=0.2)
    axr.legend(fontsize=7, loc="upper left")
